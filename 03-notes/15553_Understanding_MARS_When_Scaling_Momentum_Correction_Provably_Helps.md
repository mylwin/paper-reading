...
updated: "2026-07-24"
status: "analyzed"
source: "../02-markdown/15553_Understanding_MARS_When__2080601524336103424.md"
---
...
- **原文**：[论文 Markdown](../02-markdown/15553_Understanding_MARS_When__2080601524336103424.md)。
...
$$
\min_{x \in R^d} f(x) = E_{\xi}[f_{\xi}(x)]
$$
...
论文的核心回答是：固定 gamma 也可以分析，且合适的 gamma 位于 [0, 1] 时能降低一个由“校正误差”和“偏离 MVR 代价”共同决定的上界；但这不是对任意任务、任意 gamma 的无条件优越性承诺。
...
$$
g_t = (1-\beta)(g_{t-1}+\gamma\Delta_t)+\beta\nabla f_{\xi_t}(x_t)
$$
...
\delta_\gamma^2 =
\sup_{x \ne y}
\frac{E_{\xi}[\| \gamma d_{\xi}(x,y)-d(x,y) \|^2]}
{\| x-y \|^2}
$$
...
这表明只看相似性常数时，内点 gamma_star 的上界可同时低于两个端点对应的 L^2 与 delta^2。真正的算法复杂度还包含偏离 gamma = 1 的附加项，因此不能只最小化 delta_gamma。
...
其中 Delta = f(x_0) - f_inf，目标为使期望梯度范数平方不超过 4 epsilon^2。

这四项的解释：

- 前两项与 gamma 无关，是共同的基础复杂度。
- 第三项会随 delta_gamma 变小而降低，支持把 gamma 设为小于 1。
...
MVR 对应 J(1) = A delta。在 B < A delta 的条件下，文中给出一个位于 [0, 1] 的显式 gamma_star，并证明 J(gamma_star) 不大于 J(1)。

### 正确解读

- 结论比较的是**所展示的上界表达式**，不是证明所有训练实例中 MARS 的实际损失都严格更低。
- 条件 B < A delta 等价于 L sigma < epsilon delta。这更偏向中等目标精度、较高异质性/相似性常数的区域。
...
\hat{\gamma}_t^* =
\frac{\| d_t \|^2}
{\frac{1}{M}\sum_{m=1}^{M}\| d_{B_m,t} \|^2}
$$
...
1. **理论优势依赖条件和上界松紧度**：J(gamma_star) 不大于 J(1) 是替代上界上的比较，实际 delta_gamma 是否紧贴其上界、实际训练是否对应理论区域，均未由定理自动保证。
...
6. **理论假设与深度网络现实的距离**：全局 L-光滑、有界噪声方差、全局/最坏情况相似性常数在实际深网训练中难以直接验证；局部代理量只是诊断，并非全局 delta_gamma 的一致估计。

## 可复用的实践启示

- 把 gamma 当作一等超参数，而非默认设为 MVR 的 gamma = 1。
- 若实现允许，先做对数尺度的小 gamma 扫描，并同时观察早期 token efficiency、末期验证损失和稳定性。
- 使用局部梯度差诊断时，可估计 gamma_hat_t_star；但该量需要额外 mini-batch / 全梯度近似，只适合作为研究或离线调参信号。
...
4. **从局部统计到全局理论**：解释局部 gamma_hat_t_star 与可观测训练稳定性之间的关系，评估其在非平稳训练轨迹中的可靠性。

...
"""
关键词链接脚本
用于 start-my-day skill，在文本中查找关键词并替换为标准 Markdown 链接
"""
        ...
        if matches:
            ...
            if note_paths:
                ...
                for match in reversed(matches):
                    ...
                    markdown_link = f'[{original_text}]({note_path})'
...
def link_keywords_in_file(
    ...
    # 索引中的路径以工作区根目录为基准，转换成相对于输出文件的链接。
    ...
    for keyword, paths in keyword_index.items():
        ...
        for note_path in paths:
            target = Path(note_path).expanduser()
            if not target.is_absolute():
                target = workspace_root / target
            try:
                relative = os.path.relpath(target.resolve(), output_path.parent)
            except ValueError:
                relative = str(target)
            relative_paths.append(Path(relative).as_posix())
        relative_index[keyword] = relative_paths

    # 处理每行
    processed_lines = []
    for original_line, line_type, line_content, in_frontmatter in lines:
        if line_type in ['frontmatter', 'code', 'image', 'link', 'heading']:
            # 不处理 frontmatter、代码块、图片、已有链接和标题
            ...
            # 处理普通文本
            processed_line = link_keywords_in_text(line_content, relative_index)
    ...
    logger.info("  Original Markdown links: %d", original_links)
    logger.info("  New Markdown links: %d", new_links)
    logger.info("  Added Markdown links: %d", added_links)
...
def main():
    ...
    parser.add_argument('--workspace', type=str, default=None,
                        help='Workspace root used to resolve indexed note paths (default: current directory)')
    ...
    link_keywords_in_file(args.input, args.output, keyword_index, args.workspace)

...
def title_to_note_filename(title: str) -> str:
    """将论文标题转换为 Markdown 笔记文件名（与笔记生成脚本保持一致）。

    使用相同的规则，确保 paper-conf 生成的 Markdown 链接路径能正确指向已有的论文笔记。
    """
...
# ---------------------------------------------------------------------------
# 复用 paper-daily 的评分函数 + 共享配置解析
# ---------------------------------------------------------------------------
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# _SCRIPT_DIR = <root>/paper-conf/scripts，故共享根目录需要回退两层
_PAPER_SKILLS_ROOT = os.path.dirname(os.path.dirname(_SCRIPT_DIR))
_PAPER_DAILY_SCRIPTS = os.path.join(_PAPER_SKILLS_ROOT, 'paper-daily', 'scripts')
if _PAPER_DAILY_SCRIPTS not in sys.path:
    sys.path.insert(0, _PAPER_DAILY_SCRIPTS)
...
from paper_config import load_config as _load_shared_config, collect_keywords as _collect_shared_keywords

# 兼容旧名（部分代码/文档可能引用）
_START_MY_DAY_SCRIPTS = _PAPER_DAILY_SCRIPTS


def _force_utf8_stdio():
    """Windows 控制台默认 GBK，打印含中文标题的 JSON 会抛 UnicodeEncodeError。"""
...
# DBLP 现在用 Anubis 类 JS 挑战拦截非浏览器流量；被拦时给出可操作的提示
DBLP_HEADERS = {"User-Agent": "ConfPapers/1.0 (+paper-skills)"}
DBLP_CHALLENGE_HINT = "Making sure you're not a bot"
    ...
    for query_str in queries_to_try:
        ...
        while hits_fetched < max_results:
            ...
            for attempt in range(max_retries):
                try:
                    if HAS_REQUESTS:
                        resp = requests.get(url, headers=DBLP_HEADERS, timeout=60)
                        resp.raise_for_status()
                        text = resp.text
                    else:
                        req = urllib.request.Request(url, headers=DBLP_HEADERS)
                        with urllib.request.urlopen(req, timeout=60) as response:
                            text = response.read().decode('utf-8')

                    # DBLP 现在会用 Anubis 类 JS 挑战拦截非浏览器流量，
                    # 此时返回的是 HTML 而不是 JSON。这里给出可操作的报错，
                    # 而不是让它退化成反复重试 + "Expecting value" 噪音。
                    if not text.lstrip().startswith('{'):
                        if 'not a bot' in text or 'anubis' in text.lower():
                            raise RuntimeError(
                                "DBLP 返回了反爬验证页（%s），当前网络无法直接访问 DBLP API。"
                                "可改用 paper-daily 的 openreview 源检索顶会论文。"
                                % DBLP_CHALLENGE_HINT
                            )
                        raise RuntimeError("DBLP 返回了非 JSON 响应：%s" % text[:120].replace('\n', ' '))
                    ...
                    if isinstance(e, RuntimeError) and '反爬验证页' in str(e):
                        # 反爬拦截不会因为重试而变化，直接放弃该会议，避免无意义的等待
                        logger.error("[DBLP] %s", e)
                        query_failed = True
                        blocked = True
                        break
...
def load_conf_papers_config(config_path: str) -> Dict:
    ...
    try:
        ...
        logger.error("Error loading paper-conf config: %s", e)
        raw = {}
        config_path = None

    cp = raw.get('conf') or raw.get('conf_papers') or {}

    excluded = cp.get('excluded_keywords')
    if excluded is None:
        excluded = raw.get('excluded_keywords', [])

    keywords = cp.get('keywords') or []
    if keywords:
        logger.info("Using %d keywords from conf.keywords", len(keywords))
    else:
        keywords = raw.get('keywords', [])
        if keywords:
            logger.info("Using %d keywords from legacy top-level keywords", len(keywords))

    if not keywords:
        # 默认：与 paper-daily 同源的研究领域关键词
        try:
            shared = _load_shared_config(None)
            keywords = _collect_shared_keywords(shared)
            if keywords:
                logger.info("Using %d keywords from shared research_domains", len(keywords))
        except Exception as e:
            logger.warning("Shared config not available, using skill config only: %s", e)

    return {
        'keywords': keywords,
        'excluded_keywords': excluded,
        'default_year': cp.get('default_year') or raw.get('default_year'),
        'default_conferences': cp.get('default_conferences') or raw.get('default_conferences'),
        'top_n': cp.get('top_n') or raw.get('top_n') or 10,
    }
...
def main():
    # 默认配置路径：本 skill 目录下的 config.yaml（可用 --config 覆盖）
    default_config = os.path.join(os.path.dirname(_SCRIPT_DIR), 'config.yaml')

    parser = argparse.ArgumentParser(description='Search top conference papers via DBLP + Semantic Scholar')
    parser.add_argument('--config', type=str,
                        default=default_config,
                        help='Path to paper-conf config.yaml (default: skill-local config.yaml)')
    ...
    for p in top_papers:
        ...
        # 这样 conf-papers 生成的 Markdown 链接可以直接使用此字段，无需自行推断

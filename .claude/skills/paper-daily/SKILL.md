...
vault 路径：配置的 `vault_path` → `OBSIDIAN_VAULT_PATH` 环境变量。
...
### 格式规则（paper-reading 不是 Obsidian 工作区）

- **链接一律用标准 Markdown 相对路径**：`[PDF](01-raw/{标题}.pdf)`、`[精读](03-notes/{标题}/精读.md)`
- **禁止 Obsidian 专属语法**：不用 `![[filename.png|600]]` 嵌入、不用 `[[wikilink]]`、不写 `tags:` frontmatter（`.AGENT.md` 明确要求"不依赖特定笔记软件语法"）
- 图片用标准语法 `![说明](相对路径)`
- 无数据用 `--`，不要用 `---`（会被当分隔线）
...
```bash
python scripts/scan_existing_notes.py --vault "$OBSIDIAN_VAULT_PATH" --output existing_notes_index.json
python scripts/link_keywords.py --index existing_notes_index.json --input <笔记> --output <笔记>
```

会跳过 `index.md` 与 `images/` 下的图片索引。**注意**：`link_keywords.py` 产出的是 Obsidian wikilink，在 paper-reading 工作区里一般**不要**对日报使用（与标准 Markdown 约定冲突）。
...
- vault 中存在 `01-raw` / `02-markdown` / `03-notes` / `08-daily` 目录

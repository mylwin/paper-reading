...
def resolve_vault_path(config: dict, vault: Optional[str] = None) -> Path:
    """解析 vault 路径：显式参数 > 配置 > 环境变量。"""
    raw = vault or config.get('vault_path') or os.environ.get('OBSIDIAN_VAULT_PATH') or ''
    if not raw:
        raise ValueError(
            "未指定 Obsidian vault 路径。请在 config.yaml 中设置 vault_path，"
            "或设置 OBSIDIAN_VAULT_PATH 环境变量，或传入 --vault 参数。"
        )
    return Path(str(raw)).expanduser()


def resolve_path(config: dict, key: str, default=None, base: Optional[Path] = None, vault: Optional[Path] = None) -> Path:
    """解析配置里的路径项。

    以 `@vault/` 开头 -> 相对 vault 根目录；否则相对 base（默认 skill 目录）；
    绝对路径直接使用。`@vault/` 是 paperread 工作区用法的推荐写法。
    """
    raw = config.get(key)
    ...
    if text.startswith('@vault/'):
        if vault is None:
            raise ValueError("配置项 %s 使用了 @vault/ 前缀，但未解析出 vault 路径" % key)
        return (vault / text[len('@vault/'):]).resolve()
...
def resolve_vault_subdir(config: dict, key: str, default: str, vault: Path) -> Path:
    """解析 vault 内的子目录（如 01-raw / 03-notes / 08-daily）。
...
def scan_known_papers(vault: Path, config: dict, log=None):
    ...
    notes_dir = resolve_vault_subdir(config, 'notes_dir', '03-notes', vault)

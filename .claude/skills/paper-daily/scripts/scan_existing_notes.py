...
def _is_auxiliary_note(md_file: Path) -> bool:
    """判断是否为附属文件（图片目录索引、汇总索引等），这些不作为论文笔记。"""
    if md_file.name.lower() in ('index.md', '_index.md', 'readme.md'):
        return True
    # images/ 等子目录下生成的索引
    parent_names = {part.lower() for part in md_file.parts[:-1]}
    if 'images' in parent_names or 'figures' in parent_names:
        return True
    return False
...
def scan_notes_directory(papers_dir: Path) -> List[Dict]:
    ...
    # 否则会污染关键词索引，产生 [[index]] 之类的假链接。
    for md_file in papers_dir.rglob('*.md'):
        if _is_auxiliary_note(md_file):
            continue
        try:
...
def main():
    ...
    # 未指定时回落到配置文件
    if not args.vault or not args.papers_dir:
        try:
            sys.path.insert(0, str(Path(__file__).resolve().parent))
            from paper_config import load_config, resolve_vault_path

            config = load_config(None)
            if not args.papers_dir:
                args.papers_dir = config.get('papers_dir') or '20_Research/Papers'
            if not args.vault:
                args.vault = str(resolve_vault_path(config, None))
        except Exception as e:

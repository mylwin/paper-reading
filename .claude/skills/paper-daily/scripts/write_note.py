...
def main():
    ...
    parser.add_argument('--vault', type=str, default=None,
                        help='Obsidian vault path (default: config vault_path or OBSIDIAN_VAULT_PATH)')
    ...
    # 输出目录：<vault>/<daily_dir>/<日期>/
    base = resolve_vault_subdir(config, 'daily_dir', '08-daily', vault)
    if args.daily_dir:
        override = Path(str(args.daily_dir)).expanduser()
        base = override if override.is_absolute() else (vault / override)

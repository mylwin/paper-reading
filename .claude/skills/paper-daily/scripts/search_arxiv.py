...
def main():
    ...
    parser.add_argument('--vault', type=str, default=None,
                        help='Vault root path (default: config vault_path or OBSIDIAN_VAULT_PATH)')
    ...
    if args.exclude_known or config.get('exclude_known'):
        try:
            from paper_config import resolve_vault_path

            vault_root = resolve_vault_path(config, args.vault)
            known = scan_known_papers(vault_root, config, log=logger.info)
    ...
    # 补充稳定主干与 wikilink 文件名（旧字段名保留，避免破坏既有用法）

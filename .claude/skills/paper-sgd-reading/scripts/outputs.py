...
配置查找顺序（第一个存在的生效）：
  ...
  5. 从 `$OBSIDIAN_VAULT_PATH` 反推 `<vault>/../paper-skills/config.yaml`
  6. 从当前工作目录逐级向上找 `paper-skills/config.yaml`

vault 路径：配置的 `vault_path` -> `$OBSIDIAN_VAULT_PATH`。
...
def _config_candidates(explicit=None):
    ...
    顺序要点：**眼下安装位置旁边的配置优先于靠 vault 反推的配置**，
    这样 skill 装到 `.claude/skills/` 时会用 `.claude/skills/config.yaml`，
    而不会去命中机器上别处的另一份副本。
    """
    candidates = []
    if explicit:
    ...
    # 3) 由 vault 反推：<vault>/../paper-skills/config.yaml
    vault_env = os.environ.get('OBSIDIAN_VAULT_PATH')
    if vault_env:
        candidates.append(Path(vault_env).expanduser().parent / 'paper-skills' / 'config.yaml')
...
def resolve_vault_path(config, vault=None):
    raw = vault or config.get('vault_path') or os.environ.get('OBSIDIAN_VAULT_PATH') or ''
    if not raw:
        raise ValueError(
            '未指定论文工作区路径。请在 config.yaml 设置 vault_path，'
            '或设置 OBSIDIAN_VAULT_PATH 环境变量，或传入 --vault。'
        )
...
def resolve_vault_subdir(config, key, vault):
    """目录名来自配置；缺失时用该目录的标准名。"""
...
def resolve_targets(config, vault, title, kind, name=None):
    stem = sanitize_paper_title(title)
    if not stem:
        raise ValueError('论文标题无法生成有效主干')

    if kind == 'process':
        base = resolve_vault_subdir(config, 'reading_dir', vault)
        return {
            'kind': 'process',
            'stem': stem,
            'dir': base / stem,
    ...
    base = resolve_vault_subdir(config, 'equation_dir', vault)
...
def collect_assets(config, vault, title):
    ...
    for key, label in (('equation_dir', '公式推理'), ('translation_dir', '翻译'),
        ...
        folder = resolve_vault_subdir(config, key, vault) / stem
...
def emit_asset_links(config, vault, title, from_path=None):
    """生成可粘贴的"论文资产"小节（标准 Markdown 相对链接）。只输出文本，不改文件。"""
    stem, found = collect_assets(config, vault, title)
    if not found:
        return '', stem

    origin = Path(from_path).parent if from_path else vault
    lines = ['## 论文资产', '']
...
def main():
    ...
    try:
        ...
        return 1
    if not vault.is_dir():
        print('ERROR: 论文工作区路径不存在：%s' % vault, file=sys.stderr)
        return 1
    ...
    try:
        targets = resolve_targets(config, vault, args.title, args.kind, args.name)
    except ValueError as e:
        print('ERROR: %s' % e, file=sys.stderr)
        return 1

    if args.emit_asset_links:
        block, stem = emit_asset_links(config, vault, args.title, args.from_path)
        if args.json:
    ...
    if args.append or args.append_file:
        ...
        result.update({'config_path': str(config_path), 'vault': str(vault)})

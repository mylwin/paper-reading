...
def git_added_papers(workspace: Path, since: date, until: date):
    ...
    if not (workspace / '.git').is_dir():
        result['error'] = 'workspace 不是 git 仓库'
        return result
...
def _resolve_input_dir(config: dict, workspace: Path, rel: str) -> Path:
    """inputs 里可能混有 08-daily 与普通素材目录。"""
    if rel == DAILY_INPUT or rel == (config.get('daily_dir') or DAILY_INPUT):
        return resolve_workspace_subdir(config, 'daily_dir', DAILY_INPUT, workspace)
    return resolve_workspace_subdir(config, rel, rel, workspace)
...
def scan_context_assets(workspace: Path, config: dict, stems):
...
def collect_materials(config: dict, workspace: Path, since: date, until: date, history_size: int):
    ...
    for order, rel in enumerate(inputs):
        ...
        directory = _resolve_input_dir(config, workspace, rel)
    ...
    if weekly.get('use_git_history', True):
        git_info = git_added_papers(workspace, since, until)        # 用 git 结果补充/标注素材（git 能捞到 mtime 已被污染的旧文件）
    ...
    assets = scan_context_assets(workspace, config, matched.keys())
...
def main():
    ...
    materials = collect_materials(config, workspace, since, today, history_size)
    research_dir = resolve_workspace_subdir(config, 'research_dir', '07-research', workspace)
    manifest = {
        'skill': 'paper-weekly',

...
A. Chaudhry<sup>†</sup> K. Scheinberg<sup>‡</sup> Scholar Sun<sup>‡</sup> 
...
1 Introduction. In this paper, we focus on the complexity of model-based derivative free algorithms that aim to solve unconstrained nonlinear optimization problems of the form 

$$
\min _ {x \in \mathbb {R} ^ {n}} \phi (x),\tag{1.1}
$$
...
• In the case of linear polynomial interpolation the worst case complexity bound is $\mathcal { O } ( n ^ { 2 } \epsilon ^ { - 2 } )$ which matches those of other DFO methods, such as direct search or methods based on finite diference gradient approximation. 
...
• The method in [11] still departs from Powell’s method in its geometry handling approach. We will explain the details of this when we describe the algorithm but the key diference is that the method in [11] discards some potentially useful sample points, which Powell’s methods do not. Including these points complicates the theory substantially, but we are able to provide such theory here. 

• We derive the theory based on linear interpolation since this is suficient for the first-order convergence analysis, however, with an easy modification we allow for quadratic models without any additional complexity cost. 
...
1.1 Preliminaries The following are the standard assumptions on $\phi ( x )$ for our setting. 

Assumption 1.1 (Lower bound on $\phi )$ . The function $\phi$ is bounded below by a scalar $\phi ^ { \star }$ on $\mathbb { R } ^ { n }$ 

Assumption 1.2 (Lipschitz continuous gradient). The function ϕ is continuously diferentiable, and the gradient $o f \phi \ i s \ L \ – L i p s c h i t z$ continuous on $\begin{array} { r } { \mathbb { R } ^ { n } , \ i . e . , \ \| \nabla \phi ( y ) - \nabla \phi ( x ) \| \leq L \| y - x \| \ f o r \ a l l \ ( y , x ) \in \mathbb { R } ^ { n } \times \mathbb { R } ^ { n } } \end{array}$ 

Throughout the paper, we assume that we do not have access to $\nabla \phi ( x )$ or its approximation using any form of diferentiation. Instead we have access to an inexact zeroth-order oracle $f ( x ) \approx \phi ( x )$ . Specifically, for all x we assume 
...
$$
m _ {k} (x _ {k} + s) = \phi (x _ {k}) + g _ {k} (x _ {k}) ^ {T} s + \frac {1}{2} s ^ {T} H _ {k} (x _ {k}) s.\tag{2.1}
$$
...
Assumption $2 . 2 . \quad 1 .$ The trust region subproblem is solved suficiently accurately in each iteration k so that $x _ { k } + s _ { k }$ provides at least a fraction of Cauchy decrease, i.e. for some constant $0 < \kappa _ { f c d } < 1$ 
...
$$
\begin{array}{c} \mathcal {S} _ {\epsilon} := \{k \in \{0, \ldots , K _ {\epsilon} - 1 \}: \text {iteration k is successful} \}. \\ \mathcal {M} _ {\epsilon} := \{k \in \{0, \ldots , K _ {\epsilon} - 1 \}: \text {iteration k is model improving} \}. \\ \mathcal {U} _ {\epsilon} := \{k \in \{0, \ldots , K _ {\epsilon} - 1 \}: \text {iteration k is unsuccessful} \}. \end{array}\tag{2.3}
$$

We next present two lemmas that are key in the analysis of any trust region algorithm and specifically Algorithm 1. The first lemma establishes that once $\Delta _ { k }$ is suficiently small compared to the gradient norm, a successful step is ensured. 
...
$$
\sqrt {\frac {2 \epsilon_ {f}}{C _ {0}}} \leq \Delta_ {k} \leq C _ {1} \| \nabla \phi (x _ {k}) \|, w h e r e C _ {1} = \left(\max \left\{\eta_ {2}, \kappa_ {b h m}, \frac {2 \kappa_ {e f} + C _ {0}}{(1 - \eta_ {1}) \kappa_ {f c d}} \right\} + \kappa_ {e g}\right) ^ {- 1},\tag{2.4}
$$
...
$$
\phi (x _ {k}) - \phi (x _ {k + 1}) \geq C _ {2} \Delta_ {k} ^ {2} - 2 \epsilon_ {f}, w h e r e C _ {2} = \frac {\eta_ {1} \eta_ {2} \kappa_ {f c d}}{2} \min \{\frac {\eta_ {2}}{\kappa_ {b h m}}, 1 \};\tag{2.5}
$$
...
Theorem 2.8. Let Assumptions 1.2 and 2.2 hold. For any $\epsilon > \sqrt { \frac { 4 \epsilon _ { f } } { \gamma ^ { 2 } C _ { 2 } C _ { 1 } ^ { 2 } } }$ , assuming that the initial trustregion radius $\Delta _ { 0 } > \gamma C _ { 1 } \epsilon ,$ , where $C _ { 1 }$ is defined as $\begin{array} { r } { \bigg ( \operatorname* { m a x } \left\{ \eta _ { 2 } , \ \kappa _ { b h m } , \ \frac { 2 \kappa _ { e f } + \operatorname* { m a x } \{ \eta _ { 2 } , \kappa _ { e f } \} } { ( 1 - \eta _ { 1 } ) \kappa _ { f c d } } \right\} + \kappa _ { e g } \bigg ) ^ { - 1 } } \end{array}$ , then we have the bound 

$$
| \mathcal {S} _ {\epsilon} | + | \mathcal {U} _ {\epsilon} | \leq \frac {4 (\phi (x _ {0}) - \phi^ {\star})}{C _ {2} (\gamma C _ {1} \epsilon) ^ {2}} + \lceil \left(\log_ {\gamma} \frac {C _ {1} \epsilon}{\Delta_ {0}}\right) \rceil .\tag{2.7}
$$

Constants $C _ { 1 }$ and $C _ { 2 }$ have a direct efect on the complexity and we will be using them (and their variations) throughout the paper. Let us pause here to understand their diferent components. Specifically, constants γ, η<sub>1</sub> and $\kappa _ { f c d }$ are usually chosen to be fixed in the algorithm and be close to 1 (say 0.9). 
...
$$
\| \nabla m (x) - \nabla \phi (x) \| \leq \kappa_ {e g} \Delta\tag{2.8}
$$
...
Let us consider a concrete way of building fully-linear models and the complexity implications. From analysis of the finite diference gradient approximation error (see e.g.[4]), if one forms a gradient estimate via 

$$
g (x) = \sum_ {i = 1} ^ {n} \frac {f (x + \delta u _ {i}) - f (x)}{\delta} u _ {i},\tag{2.9}
$$
...
3 Lagrange polynomials and fully-linear models. Before introducing the method we wish to analyze in this paper we need to discuss an important tool utilized by these algorithms - Lagrange polynomials. The concepts and the definitions below can be found in [13]. 
...
$$
g _ {k} ^ {\top} y + \frac {1}{2} y ^ {\top} H _ {k} y = \phi (x _ {k} + y) - \phi (x _ {k}), \quad \forall y \in \mathcal {Y} _ {k}.\tag{3.1}
$$

The model $m _ { k } ( x )$ is then defined as 

$$
m _ {k} (x _ {k} + s) = \phi (x _ {k}) + g _ {k} ^ {T} s + \frac {1}{2} s ^ {T} H _ {k} s.\tag{3.2}
$$
...
$$
g _ {k} ^ {\top} y + \frac {1}{2} y ^ {\top} H _ {k} y = f (x _ {k} + y) - f (x _ {l}) \quad \forall y \in \mathcal {Y} _ {k}.\tag{3.3}
$$
...
This theorem allows us to conclude that our model is fully linear when our interpolation set is Λ-poised and when $\Delta$ is suficiently large. 
...
$$
\min _ {g, H} \sum_ {z \in \mathcal {Z} _ {k}} \left(f (x _ {k}) + g ^ {\top} z + \frac {1}{2} z ^ {T} H z - f (x _ {k} + z)\right) ^ {2}\tag{4.2}
$$

subject to: ∥H∥ ≤ K 

$$
g ^ {\top} y + \frac {1}{2} y ^ {\top} H y = f (x _ {k} + y) - f (x _ {k}), \quad \forall y \in \mathcal {Y} _ {k}.\tag{4.3}
$$
...
$$
^ {a} \text {   If   } \mathcal {Z} _ {k} \neq \emptyset
$$

$$
\left\| y _ {j _ {k} ^ {*}} - s _ {k} \right\| > 0
$$
...
$$
m _ {k} (x _ {k} + Q _ {k} v) = \phi (x _ {k}) + g _ {k} ^ {T} Q _ {k} v + \frac {1}{2} v ^ {T} Q _ {k} ^ {T} H _ {k} Q _ {k} v.\tag{5.1}
$$
...
$$
m (x + Q v) = \phi (x) + g ^ {T} Q v + \frac {1}{2} v ^ {T} Q ^ {T} H Q v\tag{5.2}
$$
...
$$
\| \nabla \hat {m} (0) - \nabla \hat {\phi} (0) \| \leq \kappa_ {e g} \Delta\tag{5.3}
$$
...
We will assume, as before, that Assumption 2.2 holds. The two key modifications of Algorithm 3 compared to Algorithm 1 are the additional term $2 \epsilon _ { f }$ in the definition of $\rho$ and the imposed lower bound on the trust region radius $\Delta _ { m i n }$ . Both of these modifications are needed because the noisy zeroth-order oracle makes it essential for $\Delta _ { k }$ to remain suficiently positive to ensure fully linear models. Such a lower bound on $\Delta _ { k }$ is ensured in the deterministic framework of Algorithm 1 for suficiently large $\epsilon .$ But this is not the case when random subspaces are used because unsuccessful iterations can occur even if $\Delta _ { k }$ is small, due to the subspace not being chosen well. We note that since $\epsilon _ { f }$ is an upper bound on the error in the zeroth order oracle, any upper estimate of it can be used in the algorithm. Of course, unnecessarily large values will have an adverse efect on the resulting best achievable accuracy ϵ. Similarly, we will see that the choice of $\epsilon _ { f }$ dictates the best choice for $\Delta _ { m i n }$ 
...
In order to use this theorem for efective complexity bounds, we must specify how to form models in a subspace. In the following subsections we discuss the two diferent approaches we used in the full space case - finite diferences and interpolation based Λ-poised sets. The key diference now is in the lower bound on $\Delta _ { k }$ imposed by $\Delta _ { m i n }$ rather than occurring automatically. 

5.2 Building models in a subspace One can form a gradient estimate via a subspace version of (2.9). This can take the following form given in [11]: 

$$
\hat {g} (0) = \sum_ {i = 1} ^ {q} \frac {f (x + \delta Q u _ {i}) - f (x)}{\delta} u _ {i}\tag{5.10}
$$
...
5.3 Geometry-correcting algorithm in subspaces. We now describe a geometry-correcting version of Algorithm 3. This algorithm performs model improving steps of Algorithm 2 until either successful step is achieved or a fully linear model in the subspace is formed. At that point it terminates the work in that subspace and regenerates a new subspace as well as restarts the models using the initial sample sets $\mathcal { V } _ { 0 } , \mathcal { Z } _ { 0 }$ . This initialization choice is somewhat arbitrary and can be replaced by diferent initial sets. Each time, however, this requires computation of new function values for all points in the “initial” sample set. Our computational results show that this is quite expensive, if we use $\mathcal { V } _ { 0 }$ and $\mathcal { Z } _ { 0 }$ that contain $q$ points in each. We can delay resampling the random subspace until several successful or unsuccessful steps have been encountered and extend the theory to such strategies. However our computational results so far do not support an advantage of this approach. We can also choose ${ \mathcal { V } } _ { 0 }$ to contain only 1 point and $\mathcal { Z } _ { 0 }$ to be empty by modifying model improvement step and Lagrange polynomial computation to allow for incomplete sets. This modification is simple from the theory point of view but whether it can be practically competitive is yet unclear. Thus we retain the simplest approach for our analysis. 
...
$$
\min _ {g, H} \| H _ {p r e v} - H \| _ {F} \quad \text {s.t.} \quad v ^ {\top} g + \frac {1}{2} v ^ {\top} H v = f (x _ {k} + v) - f (x _ {k}), v \in \mathcal {Y} \cup \mathcal {Z}.\tag{6.1}
$$
...
6.2 Testing and Results We now turn to the results. We state upfront that our numerical comparison yields the following observations. 
...
<table><tr><td></td><td>GC-YZ-LIN/V</td><td>GC-sub</td><td>DFOTR</td></tr><tr><td>Acceptance ratio <eq>\eta_1</eq></td><td>0.01</td><td>0.1</td><td>0.05</td></tr><tr><td>Model gradient threshold <eq>\eta_2</eq></td><td><eq>5 \times 10^{-9}</eq></td><td><eq>5 \times 10^{-9}</eq></td><td>—</td></tr><tr><td>“Very successful iteration” threshold</td><td>—</td><td>—</td><td>0.5</td></tr><tr><td>Expansion factor <eq>\gamma_{\text{inc}}</eq></td><td>1.3</td><td>1.3</td><td>1.6</td></tr><tr><td>Contraction factor <eq>\gamma_{\text{dec}}</eq></td><td>0.8</td><td>0.8</td><td>0.8</td></tr><tr><td>Resolution shrink <eq>\theta</eq></td><td>0.1</td><td>—</td><td>—</td></tr><tr><td>Poisedness threshold <eq>\Lambda</eq></td><td>1000</td><td>1000</td><td>—</td></tr><tr><td>Self-correcting threshold <eq>\Lambda_{\text{sc}}</eq></td><td>2</td><td>2</td><td>—</td></tr><tr><td>Hedge: EWMA decay <eq>\beta</eq></td><td>0.8</td><td>—</td><td>—</td></tr><tr><td>Model Hessian bound <eq>K</eq></td><td><eq>10^{100}</eq></td><td>—</td><td>—</td></tr><tr><td>Small step gate threshold <eq>c_g</eq></td><td>0.5</td><td>—</td><td>—</td></tr><tr><td>Subspace dimension <eq>q</eq></td><td>—</td><td>5</td><td>—</td></tr><tr><td>Interpolation set capacity</td><td>—</td><td>2q</td><td>—</td></tr></table>

![image](https://cdn-mineru.openxlab.org.cn/result/2026-09-17/bbcf8205-f525-44f1-b045-6cce953a6f01/be4b14033448f48d87e811b4ed01c8106e0e8d3fe71d3a65a77dc7da216a6ef9.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-09-17/bbcf8205-f525-44f1-b045-6cce953a6f01/5fbcf7e93237168be387b9c018330c7159b2d6339c39148a1a9d2a13f97d43f5.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-09-17/bbcf8205-f525-44f1-b045-6cce953a6f01/494c33de4172e7ab97ccea7136998969b84a34e6a52ef90c5b86de05d560928a.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-09-17/bbcf8205-f525-44f1-b045-6cce953a6f01/b18a78623a649dbca31596d800bea64c9885e60ff82d3f9604cd1388ae1817b2.jpg)



Figure 6.4: Data profiles for the test suite $n = 1 0 0 .$
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-09-17/bbcf8205-f525-44f1-b045-6cce953a6f01/54f1598cd40ef08f6676e4fb64cea370f7cf179d556d27276abaf5cfc036e434.jpg)



Figure 6.5: Data profiles for the test suite $n = 2 0 0 .$


Acknowledgments This work was partially supported by ONR award N00014-22-1-215 and the Gary C. Butler Family Foundation. 
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-09-17/bbcf8205-f525-44f1-b045-6cce953a6f01/e2cfeabcc71fde484c2a44fe6193fb5932bf9919f39e3f84110a322ae1e1de73.jpg)



Figure 6.6: Data profiles for the random subspace implementation compared against GC-YZ-LIN for $n = 3 0 , 1 0 0 .$
